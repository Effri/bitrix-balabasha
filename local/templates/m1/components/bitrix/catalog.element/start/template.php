...
if (\Bitrix\Main\Loader::IncludeModule("profitkit.shopstart"))
    $price = Profitkit\Shopstart\Shop::getProductPrice($actualItem);
        ...
        <section class="item">
            ...
            <div class="description">
                ...
                <div class="description__buttons">
                    <div data-entity="main-button-container">
                        <div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : '')?>;">
                            <?
                            if ($showAddBtn)
                            {
                                ?>
                            ...
                            if ($showBuyBtn)
                            {
                                ...
                                <div class="product-item-detail-info-container">
                                    <a class="btn <?=$buyButtonClassName?> product-item-detail-buy-button" id="<?=$itemIds['BUY_LINK']?>"
                                       data-toggle="pkModal" data-target="#callbackModal" data-ajax="/ajax/form.php?form_id=<?=$arParams['ONECLICK_FORM_ID'];?>&field=PRODUCT&value=<?=$arResult['NAME'];?>"
                                       href="javascript:void(0);">
                    ...
                    <button data-toggle="pkModal" data-target="#callbackModal" data-ajax="/ajax/form.php?form_id=<?=$arParams['ONECLICK_FORM_ID'];?>&field=PRODUCT&value=<?=$item['NAME'];?>" id="<?=$itemIds['BUY_ONE_CLICK']?>" class="btn_white">Купить в один клик</button>
                </div>
            ...
            <div class="right__section">
                <div class="right__section__brand">
                    ...
                    elseif ($arResult['PROPERTIES']['ARTICLE']) {
                        ?>
                        <p class="catalog_element_artnumber"><?=$arResult['PROPERTIES']['ARTICLE']['NAME'];?>: <span><?=$arResult['PROPERTIES']['ARTICLE']['VALUE'];?></span></p>
                        <?
                    }
                    ?>
                </div>
                <div class="right__section__availability">
                    <?=Profitkit\Shopstart\Shop::getProductAvailable($arResult)['html'];?>
                </div>
                <div class="right__section__shipping">
                    ...
                    <p><span data-toggle="pkModal" data-target="#deliveryModal" data-ajax="/ajax/delivery.php?id=<?=$item['ID'];?>">Расчет доставки</span>
                        (от 1 дня от 250 р.)</p>
                </div>
                <div class="right__section__payment">
                    ...
                    <p><span data-toggle="pkModal" data-target="#paymentModal" data-ajax="/ajax/payment.php?id=<?=$item['ID'];?>">Способы оплаты </span>
                ...
                <table>
                    <tr>
                        ...
                        if ($showBuyBtn)
                        {
                            ?>
                            <td rowspan="2" class="product-item-detail-short-card-btn"
                                data-toggle="pkModal" data-target="#callbackModal" data-ajax="/ajax/form.php?form_id=<?=$arParams['ONECLICK_FORM_ID'];?>&field=PRODUCT&value=<?=$item['NAME'];?>"
...
else
{
    ...
    $jsParams = array(
        'CONFIG' => array(
            'USE_CATALOG' => $arResult['CATALOG'],
            'SHOW_QUANTITY' => $arParams['USE_PRODUCT_QUANTITY'],
            'SHOW_PRICE' => true, //!empty($arResult['ITEM_PRICES']),
            'SHOW_DISCOUNT_PERCENT' => $arParams['SHOW_DISCOUNT_PERCENT'] === 'Y',
            'SHOW_OLD_PRICE' => $arParams['SHOW_OLD_PRICE'] === 'Y',
            'USE_PRICE_COUNT' => $arParams['USE_PRICE_COUNT'],
            'DISPLAY_COMPARE' => $arParams['DISPLAY_COMPARE'],
        ...
        'PRODUCT' => array(
            'ID' => $arResult['ID'],
            'ACTIVE' => $arResult['ACTIVE'],
            'PICT' => ($arResult['PREVIEW_PICTURE']?$arResult['PREVIEW_PICTURE']:$arResult['DETAIL_PICTURE']), //reset($arResult['MORE_PHOTO']),
            'NAME' => $arResult['~NAME'],
            'SUBSCRIPTION' => true,
