...
foreach ($params_array as $k => $v) {
    if ($request[$k]) {
        setcookie($cookie_prev.$k, $request[$k], time() + 100000, "/", $_SERVER['HTTP_HOST']);
...
$uri->addParams(array("view"=>($params_array['view'] == 'table'?"card":"table")));
$view_url = $uri->getUri();
        ...
        <?
			$intSectionID = $APPLICATION->IncludeComponent(
				...
				array(
                    ...
                    'ONECLICK_FORM_ID' => $arParams['ONECLICK_FORM_ID'],
				),
				$component
			);
			?>
