...
?>
	...
	<?if ($arItem["IS_PARENT"]):?>
        ...
        ?>
			<div class="menu_item<?=($arItem['PARAMS']['catalog'] == 'Y')?" menu_catalog":"";?> menu_has_child"><a href="<?=$arItem["LINK"]?>" class="<?if ($arItem["SELECTED"]):?>root-item-selected<?endif?>"><?=$arItem["TEXT"]?></a>
                <? if ($arItem['PARAMS']['catalog'] == 'Y') { $open_catalog = true; ?>
	...
	<?else:?>
		...
		<?if ($arItem["PERMISSION"] > "D"):?>
			...
			<?if ($arItem["DEPTH_LEVEL"] == 1):?>
				<div class="menu_item<?=($arItem['PARAMS']['catalog'] == 'Y')?" menu_catalog":"";?>"><a href="<?=$arItem["LINK"]?>" class="<?if ($arItem["SELECTED"]):?>root-item-selected<?endif?>"><?=$arItem["TEXT"]?></a></div>
			<?else:?>
